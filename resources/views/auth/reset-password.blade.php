<x-guest-layout>
    <div class="mb-6">
        ...
        <h1 class="mt-2 text-2xl font-black tracking-tight text-slate-950">Choose a new password</h1>
        <p class="mt-2 text-sm leading-6 text-slate-600">Create a new password so you can get back to your EduQuiz workspace.</p>
    </div>

    <form method="POST" action="{{ route('password.store') }}" class="space-y-4" x-data="{ submitting: false }" x-on:submit="submitting = true">
        @csrf
